...
namespace Everaldo.Cardoso.Araujo.C19BR.Framework.Conversion
{
    public static class VerifyType
    {
        ...
        public static object ConvertProperty(string property, object value, Type typeObject)
        {
            ...
            if (typeProperty == typeof(ushort?))
            {
                return ParseToNullValue(typeof(ushort?), value);
            }
            if (typeProperty == typeof(int?))
            {
                return ParseToNullValue(typeof(int?), value);
            }
            if (typeProperty == typeof(long?))
            {
                return ParseToNullValue(typeof(long?), value);
            }
            if (typeProperty == typeof(DateTime?))
            {
                return ParseToNullValue(typeof(DateTime?), value);
            }
            if (typeProperty == typeof(decimal?))
            {
                return ParseToNullValue(typeof(decimal?), value);
            }
            if (typeProperty == typeof(double?))
            {
                return ParseToNullValue(typeof(double?), value);
            }
            
            return value;
        }

        public static object ParseToNullValue(Type nullableType, object value)
        {
            var typeNullable = Nullable.GetUnderlyingType(nullableType);
